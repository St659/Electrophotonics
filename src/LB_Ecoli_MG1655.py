...
import numpy as np
data_directory = '../Data/LB_Ecoli_MG1655'
results_directory = '../Results/LB_Ecoli_MG1655'
...
hyperspectral_wavelengths = np.arange(810,870.5,0.5)
...
try:
    spectra_files = os.listdir(spectra_results_directory)
    spectra_file = 'default'
    try:
        spectra_files.remove('.DS_Store')
    ...
    max_wavelengths = np.load(os.path.join(spectra_results_directory,spectra_file))
except OSError:

    for directory in sorted_spectra_subdirectories:
        print(directory)
        max_wavelength = plot_spectra(os.path.join(spectra_data_directory,directory), spectra_wavelengths, spectra_plot)
        if max_wavelength > 800:
            max_wavelengths.append(max_wavelength)

    np.save(os.path.join(spectra_results_directory, 'spectra_results'),max_wavelengths)
...
for directory in sorted_results_file:
    ...
    data_slice = slice(200,-300)
    side_slice = slice (200,-350)
...
image_average.errorbar(np.arange(0,len(sorted_results_file),1)*6,mean, std,fmt='o')
